...
if __name__ == '__main__':
    ...
    dataset_path = '/data1/home/ha2/dataset/pyg_data' 
    dataset = Planetoid(dataset_path, name='Cora', transform=T.NormalizeFeatures())
    data = dataset[0].to(device)

    gcn = GCN(dataset.num_features, 16, dataset.num_classes).to(device)         # ON CORA 0.80
    gat = GAT(dataset.num_features, 16, dataset.num_classes).to(device)

    train(gcn, data)        # GCN这个直接用SGA项目的模型。。。s
    gcn.eval()
    ...
    clean_acc = test(gat, data) # Clean accuracy: 0.659        
    ...
    clean_acc = test(gcn, data)         # ON CORA 0.80
