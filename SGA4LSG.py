...
surrogate_model = Trainer(SGC(num_features, num_classes), device=device, lr=0.1,
                         weight_decay=1e-5)
ckp = ModelCheckpoint('model_before.pth', monitor='val_acc')        # pubmed 386M
surrogate_model.fit(data, mask=(splits.train_nodes, splits.val_nodes),
                   callbacks=[ckp])         # pubmed 1148M
print('On Local Surrogate Model.....')
print('Testing.....')
test_res = surrogate_model.test_step(data, mask=splits.test_nodes)
print(test_res)
surrogate_model_output = surrogate_model.predict(data, mask=target)
print(mark(surrogate_model_output, target_label))

target_model = Trainer(GCN(num_features, num_classes), device=device, lr=0.1,
                         weight_decay=1e-5)
ckp1 = ModelCheckpoint('model_before.pth', monitor='val_acc')        # pubmed 386M
target_model.fit(data, mask=(splits.train_nodes, splits.val_nodes),
                   callbacks=[ckp1])         # pubmed 1148M
print('On Remote TARGET model.....')
print('Testing.....')
test_res = target_model.test_step(data, mask=splits.test_nodes)
print(test_res)
target_model_output = target_model.predict(data, mask=target)
print(mark(target_model_output, target_label))
