...
print('Testing.....')
test_res = trainer_before.test_step(data, mask=splits.test_nodes)
print(test_res)
...
output = trainer_before.predict(attacker.data(), mask=target)           # 调用.data()函数的时候，data = copy(self.ori_data)是从原始raw graph上添加边的，之前的attack 过程已经把需要添加或者删除的边记录放进去了
